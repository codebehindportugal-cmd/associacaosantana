...
class RelatorioController extends Controller
{
    ...
    public function porPeriodo(Request $request): Response
    {
        ...
        return Inertia::render('Relatorios/PorPeriodo', [
            ...
            'resumo' => ['total_periodo' => $total, 'total_pedidos' => $pedidos->count(), 'media_diaria' => $total / $dias, 'total_doacoes' => (float) $pedidos->sum('doacao')],
    ...
    private function topProdutos($inicio, $fim, int $limite)
    {
        return DB::table('pedido_items')
            ...
            ->select('produtos.nome', 'categorias.nome as categoria', DB::raw('SUM(pedido_items.quantidade) as quantidade'), DB::raw('SUM(pedido_items.quantidade * pedido_items.preco_unitario) as total'))
