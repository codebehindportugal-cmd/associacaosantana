    ...
    <main class="min-h-screen bg-gray-900 p-5 text-white">
        <header class="mb-6">
            <div class="flex items-center justify-between gap-3">
                <h1 class="text-xl font-black sm:text-3xl">💳 TESOURARIA</h1>
                <div class="flex items-center gap-3">
                    <span class="hidden font-black sm:block">{{ agora.toLocaleTimeString('pt-PT') }}</span>
                    <button class="rounded-lg bg-amber-500 px-3 py-2 text-sm font-black text-black" @click="chamandoComissao = true">🎉 COMISSÃO</button>
                    <button class="rounded-lg bg-red-600 px-4 py-3 font-black" @click="router.post(route('pos.logout'))">LOGOUT</button>
                </div>
            </div>
            <p class="mt-1 text-sm font-bold text-gray-400">Associação de Santana · {{ agora.toLocaleTimeString('pt-PT', { hour: '2-digit', minute: '2-digit' }) }}</p>
        </header>
