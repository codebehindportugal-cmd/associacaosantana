...
const ativo = (mesa) => (mesa.pedidos ?? [])[0];
const pedidosAtivos = (mesa) => [...(mesa.pedidos ?? []), ...((mesa.submesas ?? []).flatMap((submesa) => submesa.pedidos ?? []))];
const estadoVisual = (mesa) => pedidosAtivos(mesa).length ? 'ocupada' : mesa.estado;
...
const cor = (mesa) => estadoVisual(mesa) === 'ocupada' ? 'bg-red-600' : estadoVisual(mesa) === 'reservada' ? 'bg-yellow-500 text-gray-950' : 'bg-emerald-600';
        ...
        <section v-for="(lista, local) in grupos" :key="local" class="mb-7">
            ...
            <div class="grid grid-cols-3 gap-3 sm:grid-cols-5 md:grid-cols-7 lg:grid-cols-9">
                <Link v-for="mesa in lista" :key="mesa.id" :href="route('pos.rest.mesa', mesa.id)" class="flex min-h-[116px] flex-col items-center justify-center rounded-lg p-3 text-center font-black shadow" :class="cor(mesa)">
                    ...
                    <span v-if="estadoVisual(mesa) === 'ocupada'" class="mt-1 text-sm">{{ total(mesa) }} · {{ minutos(mesa) }}</span>
                </Link>
            </div>
        </section>
    </main>
