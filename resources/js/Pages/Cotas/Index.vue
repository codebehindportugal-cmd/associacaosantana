...
<template>
    <AppLayout>
        ...
        <div class="overflow-hidden rounded-lg bg-white shadow-sm"><table class="w-full text-left text-sm"><tbody><tr v-for="cota in cotas.data" :key="cota.id" class="border-t"><td class="p-3">{{ cota.socio?.nome }}</td><td>{{ cota.ano }}/{{ cota.mes }}</td><td>{{ cota.valor }}€</td><td>{{ cota.estado }}</td></tr></tbody></table></div>
    </AppLayout>
</template>
