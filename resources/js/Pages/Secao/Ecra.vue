...
<template>
    <main class="min-h-screen bg-[#1a1a2e] p-5 text-white sm:p-8">
        <div v-if="tem_urgentes" class="mb-5 animate-pulse rounded-2xl bg-red-600 p-5 text-center text-2xl font-black text-white shadow-lg">⚡ ATENÇÃO — HÁ PEDIDOS URGENTES</div>
        ...
        <div v-else class="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
            <section v-for="grupo in itemsPorMesa" :key="grupo.mesa" class="relative rounded-lg p-6" :class="grupo.urgente ? 'border-4 border-red-500 bg-red-950/70' : 'bg-white/10'">
                <span v-if="grupo.urgente" class="absolute right-4 top-4 rounded-full bg-red-600 px-3 py-1 text-sm font-black">⚡ URGENTE</span>
                <h2 class="mb-4 text-3xl font-bold">{{ grupo.mesa }}</h2>
                <div v-for="item in grupo.items" :key="item.id" class="mb-3 flex items-center justify-between gap-4 rounded-md p-4 text-slate-900 transition-all duration-500" :class="item.prioridade ? 'bg-red-900 text-white ring-4 ring-red-500' : (item.observacoes ? 'bg-amber-100 ring-4 ring-amber-400' : (novosItems.has(item.id) ? 'bg-amber-200 ring-4 ring-amber-400' : 'bg-white'))">
                    <div>
                        <span class="block text-[1.2rem] font-bold">{{ item.quantidade }}x {{ item.produto?.nome }}</span>
                        <span v-if="item.prioridade" class="mt-1 block text-sm font-black text-red-200">⚡ URGENTE</span>
                    ...
                    <button v-if="!modoBar" class="min-h-[60px] rounded-md px-5 py-3 font-black text-white" :class="item.prioridade ? 'bg-red-500 text-lg' : 'bg-emerald-600'" @click="pronto(item.id)">PRONTO</button>
                </div>
            </section>
        </div>
