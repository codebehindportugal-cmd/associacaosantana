            ...
            <div class="pos-layout grid min-h-0 flex-1 gap-3 lg:grid-cols-[minmax(0,1fr)_380px] xl:grid-cols-[minmax(0,1fr)_410px]">
                <div class="pos-left flex min-h-0 flex-col gap-3">
                    <section class="pos-panel flex min-h-0 flex-1 flex-col rounded-lg bg-gray-800 p-3 sm:p-4">
                        <h2 class="pos-section-title mb-3 shrink-0 text-xl font-black sm:text-2xl">BEBIDAS</h2>
                        <div class="pos-product-grid grid min-h-0 flex-1 auto-rows-min grid-cols-2 gap-2 overflow-y-auto pr-1 md:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5">
                            <button v-for="produto in produtos" :key="produto.id" class="pos-product-btn min-h-20 rounded-lg bg-blue-600 p-3 text-left font-black disabled:opacity-50 sm:min-h-24" :disabled="!caixaAberta" @click="adicionar(produto)">
                ...
                <aside class="pos-cart flex min-h-0 flex-col rounded-lg bg-gray-800 p-3 sm:p-4">
                    ...
                    <div class="pos-cart-list min-h-0 flex-1 overflow-y-auto pr-1">
                        <div v-if="!carrinho.length" class="pos-empty rounded-lg bg-gray-900 p-5 text-center font-bold text-gray-300">Escolhe bebidas. Ex.: 2 Imperiais.</div>
